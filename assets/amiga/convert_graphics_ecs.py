...
# on AGA, we got away with attributes and 0x40 adds and shit to get the proper sprite value on the
...
""")
    ...
    def update_hw_status(mapping,code):
        for k,v in mapping.items():
            if isinstance(v,int):
                v = [v,None]
            for i,vi in enumerate(v):
                if vi is not None:
                    hw_status[k*4+i*2] = code
                    hw_status[k*4+1+i*2] = vi
